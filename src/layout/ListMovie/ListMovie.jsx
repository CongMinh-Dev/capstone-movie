...
const Li = () => {
  ...
  const itemsTitle = ["Gần Đây", "Đánh Giá Cao", "Lượt xem nhiều","Tất Cả"];

  // đẩy phim lên redux
  const dispatch = useDispatch();
  const handleClickView = (movie) => {
    dispatch(setMovieDetail(movie));
    document.querySelector(".info_movie_bg").style.display="block"
    document.querySelector(".info_movie").style.display="block"
  ...
  useEffect(() => {
    const fetchData = async () => {
      const promises = itemsTitle.map((title, index) =>{
        if(index<3){
          return quanLyPhimServ.getMoviePage(index + 1)
        } else{
          return quanLyPhimServ.getAllMovie()
        }
      }
      );
